"""BiTerminalTextualApp — the real, full-parity Textual renderer app.

Supersedes the tracer-bullet's placeholder driver (which only wired up the
main menu, with no cfg/client). This is a direct sync port of bi_python's
app.py flow graph — same control flow, same order of operations, same
notify messages — but calling specs.menus/specs.forms builders instead of
inline literal menu/form definitions, self.renderer.show_*/notify instead of
push_screen_wait directly, core.api directly with no asyncio.to_thread (see
below for why), and core.policy/core.models throughout so the policy fixes
built into core/ during the foundation increment actually take effect.

**No asyncio.to_thread anywhere.** bi_python wrapped every self.client.*
call in asyncio.to_thread specifically because its driver ran on the app's
own event loop — a real network call would otherwise freeze the whole UI.
Here, _run_flow (and everything it calls) already runs inside
run_worker(thread=True) (proven in the tracer bullet) — core.api calls are
just plain blocking calls now; only self.renderer.show_*/notify need to
cross back to the event loop, which they already do internally via
call_from_thread. A genuine simplification versus bi_python's app.py, not
just a port.
"""
...
from ...core.policy import ListFetchPolicy, fetch_list, submit_with_image_retry
...
from .renderer import _MODE_TO_CAPABILITY, TextualRenderer


class _Bubble(Exception):
    """Internal-only: wraps a GlobalNavigate so it can unwind _run_flow's
    nested sync call graph, exactly like bi_python's NavigateTo(Exception)
    did for its nested awaits. Never crosses the Renderer protocol boundary
    — TextualRenderer.show_action_menu still returns a plain GlobalNavigate
    value per the documented contract (renderers/base.py); only this
    app-level driver converts it into a raised exception, mirroring
    bi_python's own separation (ActionMenuScreen dismisses a value; app.py's
    show_action_menu() wrapper does the raise)."""
...
class BiTerminalTextualApp(App):
    ...
    def on_mount(self) -> None:
        self.dark = True
        self.renderer = TextualRenderer(self, image_mode=self.cfg.get("image_mode", "none"))
        self.run_worker(self._run_flow, thread=True, exclusive=True)

    # ── Driver ───────────────────────────────────────────────────────────
    # NOT named `_driver` — Textual's own App sets self._driver as an
    # instance attribute internally (the actual terminal I/O driver), which
    # silently shadows a same-named method instead of raising. Confirmed the
    # hard way in the tracer-bullet increment; see that commit's message.

    def _run_flow(self) -> None:
        if not self.cfg.get("token"):
            if not self._login_flow():
                return  # Exit already called self.exit(); stop looping.
        ...
        while True:
            try:
                ...
                self.call_from_thread(self._pop_all_screens)
    ...
    def _login_flow(self) -> bool:
        ...
        chose Exit (self.exit() already called by then)."""
        while True:
    ...
    def _edit_item(self, it: dict, bins: list) -> Optional[dict]:
        ...
        def submit():
            return self.client.update_item(
                ...
                # specs/forms.py), so FormScreen never populates the key
                # otherwise. Defaults to [] (nothing to keep), matching
                # bi_python's _ItemFormScreen, which always initialized
                # self._remaining_images to [] regardless of whether the
                # manager row was shown.
    ...
    def _settings_menu(self) -> None:
        ...
        config.save(self.cfg)
        # bi_python re-read cfg["image_mode"] fresh on every image view, so
        # a settings change took effect immediately with nothing to
        # invalidate. TextualRenderer caches image_mode/image_capability at
        # construction instead (renderer.py) — update both live here so a
        # mid-session mode change has the same immediate-effect behavior.
        self.renderer.image_mode = mode
        self.renderer.image_capability = _MODE_TO_CAPABILITY.get(mode, ImageCapability.NONE)
